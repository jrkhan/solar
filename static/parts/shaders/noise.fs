...
void main( void ) {
    ...
    float other = 1.0 - n;
    vec3 c1 = secondaryColor;
    vec3 c2 = color;
    float r = n * c1.x + other * c2.x;
    float g = n * c1.y + other * c2.y;
    float b = n * c1.z + other * c2.z;
    ...
    vLightWeighting += vec3( .5 ) * directionalLightWeighting;

    // specular light

    vec3 dirHalfVector = normalize( lDirection.xyz + normalizedViewPosition );
    ...
    // rim light
    float rimLight = 1.0 - dot(normal, pos);
    float edgeLight = 1.0 - dot(vNormal, pos);
    vec3 colorBoost = .25 * vec3(1.0,1.0,1.0) + .75 * color;
    //gl_FragColor *= vec4(vLightWeighting, 1.0 );
    //gl_FragColor += vec4(color * rimLight, 1);
    gl_FragColor = edgeLight * vec4(colorBoost,1) + (1.0 - edgeLight) * gl_FragColor;
